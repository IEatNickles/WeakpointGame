...
public class PlayerCharacterController : MonoBehaviour, ICharacterController {
  ...
  float m_lastDash = 0f;
  Vector3 m_dashDir;
  [SerializeField] float m_slideForce = 12f;
  [SerializeField] float m_slideSlopeAccel = 2f;
  [SerializeField] float m_slideStrafeForce = 3f;
  Vector3 m_slideDir;
  ...
  void OnCrouch(InputAction.CallbackContext ctx) {
    ...
    //   Motor.GroundingStatus.SnappingPrevented = true;
    //   if (m_inputDir.sqrMagnitude > 0f) {
    //     m_slideDir = (forward * m_inputDir.y + right * m_inputDir.x);
    //   } else {
    //     m_slideDir = forward;
    //   }
    //   Motor.Capsule.height = 1f;
    //   if (Motor.Velocity.magnitude < m_slideForce) {
    //     m_velocity = (m_slideDir * m_slideForce);
    //   }
    // } else {
    //   m_moveState = MovementState.Walking;
    //   Motor.Capsule.height = 2f;
    //   Motor.GroundingStatus.SnappingPrevented = false;
    // }
  }
